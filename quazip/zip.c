...
static void initZip64Support(zip64_internal* zi)
{
    if ((zi->ci.versionMadeBy & 0xFF) < 45)
    {
        zi->ci.versionMadeBy &= 0xFF00;
        zi->ci.versionMadeBy |= 45;
    }
    if (zi->ci.versionNeeded < 45)
        zi->ci.versionNeeded = 45;
}
                          ...
                          const void* extrafield_local)
{
  ...
  if (err==ZIP_OK)
      err = zip64local_putValue(&zi->z_filefunc,zi->filestream,(uLong)zi->ci.versionNeeded,2);
...
extern int ZEXPORT zipOpenNewFileInZipPassword(zipFile file,
                       const zip_fileinfo* zipfi,
                       const char *password);

extern int ZEXPORT zipOpenNewFileInZipKeys(zipFile file,
                       const zip_fileinfo* zipfi,
                                        ...
                                        const unsigned long *keys)
{
    ...
    zi->ci.raw = raw;
    zi->ci.versionMadeBy = zipfi->versionMadeBy;
    zi->ci.versionNeeded = version_to_extract;
    ...
    zi->ci.size_centralExtra = zipfi->size_extrafield_global;
    zip64local_putValue_inmemory(zi->ci.central_header,(uLong)CENTRALHEADERMAGIC,4);
    /* version info */
    zip64local_putValue_inmemory(zi->ci.central_header+4,(uLong)zipfi->versionMadeBy,2);
    zip64local_putValue_inmemory(zi->ci.central_header+6,(uLong)zi->ci.versionNeeded,2);
    ...
    zip64local_putValue_inmemory(zi->ci.central_header+30,(uLong)zipfi->size_extrafield_global,2);
    ...
    if(zi->ci.pos_local_header >= 0xffffffff)
      ...
      zip64local_putValue_inmemory(zi->ci.central_header+42,(uLong)zi->ci.pos_local_header - zi->begin_pos,4);

    char* central_header_ptr = &zi->ci.central_header[SIZECENTRALHEADER];
    memcpy(central_header_ptr, filename, size_filename);
    central_header_ptr += size_filename;
    memcpy(central_header_ptr,
            zipfi->extrafield_global, zipfi->size_extrafield_global);
    central_header_ptr += zipfi->size_extrafield_global;
    memcpy(central_header_ptr, comment, size_comment);

    zi->ci.zip64 = zipfi->zip64;
    zi->ci.totalCompressedData = 0;
    zi->ci.totalUncompressedData = raw ? zipfi->uncompressed_size : 0;
    zi->ci.pos_zip64extrainfo = 0;

    err = Write_LocalFileHeader(zi, filename, zipfi->size_extrafield_local,
                                zipfi->extrafield_local);
...
#ifndef NOCRYPT
    if (err==Z_OK && (password != NULL || keys != NULL))
    {
        zi->ci.pcrc_32_tab = get_crc_table();
        zi->ci.encrypt = 1;
        unsigned char crypt_header[RAND_HEAD_LEN];
        uLong crcForCrypting = (uLong)zi->ci.dosDate << 16;
        if (password != NULL)
        {
            crypthead(password,crypt_header,
                      zi->ci.keys,zi->ci.pcrc_32_tab,crcForCrypting);
        } else if (keys != NULL)
        {
            memcpy(zi->ci.keys, keys, sizeof(zi->ci.keys));
            crypthead_keys(crypt_header,
                          zi->ci.keys,zi->ci.pcrc_32_tab,crcForCrypting);
        }
        zi->ci.crypt_header_size = RAND_HEAD_LEN;

        if (ZWRITE64(zi->z_filefunc, zi->filestream,
                     crypt_header, RAND_HEAD_LEN) != RAND_HEAD_LEN)
            err = ZIP_ERRNO;
    }
#endif
...
extern int ZEXPORT zipOpenNewFileInZip(zipFile file,
                       const zip_fileinfo* zipfi)
{
    return zipOpenNewFileInZipInternal(file, zipfi, NULL, NULL);
}
...
extern int ZEXPORT zipOpenNewFileInZipPassword(zipFile file,
                       ...
                       const char *password)
{
    return zipOpenNewFileInZipInternal(file, zipfi, password, NULL);
}
...
extern int ZEXPORT zipOpenNewFileInZipKeys(zipFile file,
                                           ...
                                           const unsigned long *keys)
{
    return zipOpenNewFileInZipInternal(file, zipfi, NULL, keys);
}
...
extern int ZEXPORT zipCloseFileInZip (zipFile file)
{
    ...
    int zip64 = compressed_size > maxSize
          || uncompressed_size > maxSize
          || zi->ci.pos_local_header > maxSize
          || zi->number_entry > 0xFFFF;

    if (zip64 && !zi->ci.zip64)
        err = ZIP_PARAMERROR;
    ...
    if (err == ZIP_OK)
    {
        if (zip64)
        {
            ...
            zip64local_putValue_inmemory(
                        zi->ci.central_header+4,(uLong)zi->ci.versionMadeBy,2);
            zip64local_putValue_inmemory(
                        zi->ci.central_header+6,(uLong)zi->ci.versionNeeded,2);
        }
        ...
        if(compressed_size >= maxSize)
          zip64local_putValue_inmemory(zi->ci.central_header+20, maxSize,4); /*compr size*/
        else
        ...
        if(zi->ci.pos_local_header >= maxSize)
          datasize += 8;
    ...
    if (err==ZIP_OK)
    {
        if ((zi->flags & ZIP_SEQUENTIAL) == 0) {
            ...
            if (ZSEEK64(zi->z_filefunc,zi->filestream,
                        zi->ci.pos_local_header + 4,ZLIB_FILEFUNC_SEEK_SET)!=0)
                err = ZIP_ERRNO;
            ...
            if (err==ZIP_OK)
                err = zip64local_putValue(&zi->z_filefunc,zi->filestream,crc32,4); /* crc 32 */

            if (err == ZIP_OK
            && (uncompressed_size >= maxSize || compressed_size >= maxSize))
            {
                if(zi->ci.pos_zip64extrainfo > 0)
                {
                    ...
                    if (err==ZIP_OK) /* compressed size */
                        err = zip64local_putValue(&zi->z_filefunc, zi->filestream, uncompressed_size, 8);

                    if (err==ZIP_OK) /* uncompressed size */
...
{
  int err = ZIP_OK;
  ZPOS64_T pos = zip64eocd_pos_inzip - zi->begin_pos;
...
int Write_Zip64EndOfCentralDirectoryRecord(zip64_internal* zi, uLong size_centraldir, ZPOS64_T centraldir_pos_inzip)
{
  ...
  if (err==ZIP_OK)
    err = zip64local_putValue(
                &zi->z_filefunc,zi->filestream,(uLong)zi->ci.versionMadeBy,2);

  if (err==ZIP_OK)
    err = zip64local_putValue(
                &zi->z_filefunc,zi->filestream,(uLong)zi->ci.versionNeeded,2);
  ...
  {
    ZPOS64_T pos = centraldir_pos_inzip - zi->begin_pos;
  ...
  if (err==ZIP_OK) /* offset of start of central directory with respect to the starting disk number */
  {
    ...
    if(pos >= 0xffffffff)
    {
      err = zip64local_putValue(&zi->z_filefunc,zi->filestream, (uLong)0xffffffff,4);
                  ...
                  err = zip64local_putValue(&zi->z_filefunc,zi->filestream, (uLong)(centraldir_pos_inzip - zi->begin_pos),4);
  }
...
extern int ZEXPORT zipClose (zipFile file, const char* global_comment)
{
    ...
    pos = centraldir_pos_inzip - zi->begin_pos;
