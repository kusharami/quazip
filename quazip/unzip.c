...
local ZPOS64_T unz64local_SearchCentralDir(const zlib_filefunc64_32_def* pzlib_filefunc_def, voidpf filestream)
{
    ...
    uSizeFile = uEndFilePos - uBeginFilePos;
    ...
    while (uBackRead<uMaxBack)
    {
        ...
        uReadPos = uEndFilePos-uBackRead ;

        uReadSize = ((BUFREADCOMMENT+4) < (uEndFilePos-uReadPos)) ?
                     (BUFREADCOMMENT+4) : (uLong)(uEndFilePos-uReadPos);
                               ...
                               int is64bitOpenFunction, unsigned flags)
{
    ...
    else do
    {
        ...
        if (ZSEEK64(us.z_filefunc, us.filestream,
                begin_file_pos,ZLIB_FILEFUNC_SEEK_SET)!=0)
            break;
...
local void unz64local_DosDateToTmuDate (ZPOS64_T ulDosDate, tm_unz* ptm)
{
    ...
    ptm->tm_mday = 1+(uInt)((uDate&0x1f) - 1) % 31;
    ptm->tm_mon =  1+(uInt)((((uDate)&0x1E0)/0x20)-1) % 12;
    ptm->tm_year = (uInt)(((uDate&0x0FE00)/0x0200)+1980);

    ptm->tm_hour = (uInt) ((ulDosDate &0xF800)/0x800) % 24;
    ptm->tm_min =  (uInt) ((ulDosDate&0x7E0)/0x20) % 60;
    ptm->tm_sec =  (uInt) (2*(ulDosDate&0x1f)) % 60;
}
...
*/
static int unzOpenCurrentFile34 (unzFile file,
                                 int* method,
                                 int* level,
                                 int raw,
                                 const char* password,
                                 const unsigned long* keys)
{
    ...
    int encrypted = s->cur_file_info.flag & 1;
    if (encrypted && !raw)
    {
#ifdef NOUNCRYPT
        return UNZ_BADZIPFILE
    ...
    if (encrypted && (password != NULL || keys != NULL))
    {
        ...
        if (password != NULL)
            ...
            memcpy(s->keys, keys, sizeof(s->keys));
        ...
        if (ZSEEK64(s->z_filefunc, s->filestream,
                  ...
                  SEEK_SET)!=0)
            return UNZ_INTERNALERROR;
        if(ZREAD64(s->z_filefunc, s->filestream,crypt_header,
                   RAND_HEAD_LEN)<RAND_HEAD_LEN)
            return UNZ_INTERNALERROR;

        int i;
        for (i = 0; i < RAND_HEAD_LEN; i++)
        {
            unsigned char c = crypt_header[i];
            zdecode(s->keys,s->pcrc_32_tab,c);
        }
        s->pfile_in_zip_read->pos_in_zipfile+=RAND_HEAD_LEN;
        s->encrypted = 1;
    }
#endif
    if (encrypted)
...
extern int ZEXPORT unzOpenCurrentFile3 (unzFile file, int* method,
                                         ...
                                         const char* password)
{
    return unzOpenCurrentFile34(file, method, level, raw, password, NULL);
}

extern int ZEXPORT unzOpenCurrentFile4 (unzFile file, int* method,
                                            int* level, int raw,
                                         const unsigned long* keys)
{
    return unzOpenCurrentFile34(file, method, level, raw, NULL, keys);
}
...
extern int ZEXPORT unzReadCurrentFile  (unzFile file, voidp buf, unsigned len)
{
    ...
    int raw = pfile_in_zip_read_info->raw;

    pfile_in_zip_read_info->stream.next_out = (Bytef*)buf;

    pfile_in_zip_read_info->stream.avail_out = (uInt)len;

    if (!raw && (len>pfile_in_zip_read_info->rest_read_uncompressed))
        pfile_in_zip_read_info->stream.avail_out =
            (uInt)pfile_in_zip_read_info->rest_read_uncompressed;

    if (raw && (len>pfile_in_zip_read_info->rest_read_compressed+
...
#            endif
        ...
        if ((pfile_in_zip_read_info->compression_method==0) || raw)
        {
            uInt uDoCopy,i ;
