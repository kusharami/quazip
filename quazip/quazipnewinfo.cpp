...
#define UNIX_SYMLINK_ATTRIBUTE 0x2000
#define UNIX_DIRECTORY_ATTRIBUTE 0x4000
#define UNIX_FILE_ATTRIBUTE 0x8000
#define DOS_FILE_HIDDEN 0x02
#define DOS_FILE_DIRECTORY 0x10
#define DOS_FILE_READONLY 0x01

static quint32 QuaZipNewInfo_setPermissions(QuaZipNewInfo *info,
    ...
    bool isHidden = false)
{
    ...
    if (isDir) {
        result |= UNIX_DIRECTORY_ATTRIBUTE;
    } else {
        result |= UNIX_FILE_ATTRIBUTE;
    }

    if (isSymLink) {
        result |= UNIX_SYMLINK_ATTRIBUTE;
    }

    result <<= 16;

    if (0 == (perm & QFile::WriteUser)) {
        result |= DOS_FILE_READONLY;
    }
    if (isDir) {
        result |= DOS_FILE_DIRECTORY;
    }
    if (isHidden) {
        result |= DOS_FILE_HIDDEN;
    }
...
//    } else {
//        dateTime = lm;
//        QuaZipNewInfo_setPermissions(
//            this, info.permissions(), info.isDir(), info.isSymLink());
//    }
//}
...
//static void setNTFSTime(
//    QByteArray &extra, const QDateTime &time, int position, int fineTicks)
//{
...
//            (static_cast<unsigned>(static_cast<unsigned char>(extra.at(i + 1)))
//                << 8);
//        i += 2;
//        unsigned length =
//            static_cast<unsigned>(static_cast<unsigned char>(extra.at(i))) |
...
bool QuaZipNewInfo::initWithFile(const QString &filePath)
{
    QFileInfo fileInfo(filePath);

    if (fileInfo.exists()) {
        creationTime = fileInfo.created();
        modificationTime = fileInfo.lastModified();
        lastAccessTime = fileInfo.lastRead();
...
bool QuaZipNewInfo::initWithFile(
